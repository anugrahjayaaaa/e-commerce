            ...
            <div class="flex flex-col lg:flex-row gap-12">
                    ...
                    <div class="swiper-container gallery-thumbs">
                        <div class="swiper-wrapper">
                            <div
                                ...
                                <img src="{{ asset('uploads/products/thumbnails/' . $product->image) }}" class="w-full"
                                    alt="{{ $product->name }}">
                            </div>
                            @if ($product->images)
                                @foreach (explode(',', $product->images) as $img)
                                    <div
                                        class="swiper-slide cursor-pointer border rounded overflow-hidden opacity-50 hover:opacity-100 transition">
                                        <img src="{{ asset('uploads/products/thumbnails/' . $img) }}" class="w-full"
                                            alt="{{ $product->name }}">
                                    </div>
                                @endforeach
                            @endif
                ...
                <div class="w-full lg:w-1/2">
                    ...
                    <div class="flex flex-wrap items-center gap-4 mb-8">
                        ...
                        {{-- add to cart --}}
                        <button class="bg-sky-700 text-white px-8 py-3 rounded hover:bg-primary transition font-medium">
                            Add To Cart
                        </button>
                    ...
                    <div class="space-y-2 text-sm text-gray-600 border-t pt-6">
                        {{-- SKU --}}
                        <p>
                            <span class="font-bold text-gray-800 w-24 inline-block">SKU:</span>
                            {{ $product->SKU }}
                        </p>
                        {{-- brand --}}
                        <p>
                            <span class="font-bold text-gray-800 w-24 inline-block">Brand:</span>
                            <a href="#" class="hover:text-primary">{{ $product->brand->name }}</a>
                        </p>
                        {{-- category --}}
                        <p>
                            <span class="font-bold text-gray-800 w-24 inline-block">Category:</span>
                            <a href="#" class="hover:text-primary">{{ $product->category->name }}</a>
                        </p>
                        ...
                        @foreach ($relatedProducts as $relatedProduct)
                            <div class="swiper-slide">
                                <div class="group text-center">
                                    ...
                                    <h4 class="font-medium hover:text-primary">
                                        <a href="{{ route('shop.products.show', $relatedProduct->slug) }}">
                                            {{ $relatedProduct->name }}
                                        </a>
                                    </h4>
                                    <p class="text-primary font-bold mt-1">
                                        @if ($relatedProduct->sale_price && $relatedProduct->sale_price < $relatedProduct->regular_price)
                                            <span class="text-xl text-gray-400 line-through">
                                                ${{ number_format($relatedProduct->regular_price, 2) }}
                                            </span>
                                            <span class="text-2xl text-primary font-bold">
